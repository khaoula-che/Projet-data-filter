...
class DataManagerApp:
    ...
    def create_main_app(self):
        ...
        tk.Button(button_frame, text="Afficher Stats", command=lambda: show_stats(self.data)).grid(row=0, column=1,
                                                                                                   padx=10)
        tk.Button(button_frame, text="Filtrer Données", command=lambda: filter_data(self)).grid(row=0, column=2,
                                                                                                padx=10)
        tk.Button(button_frame, text="Trier Données", command=lambda: sort_data(self)).grid(row=0, column=3, padx=10)

        tk.Button(button_frame, text="Annuler Tout", command=lambda: reset_all(self)).grid(row=1, column=0, padx=10,
                                                                                           pady=10)
        tk.Button(button_frame, text="Annuler le Tri", command=lambda: reset_sort(self)).grid(row=1, column=1, padx=10,
                                                                                              pady=10)
