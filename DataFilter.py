...
class DataManagerApp:
    ...
    def create_welcome_screen(self):
        ...
        welcome_label = tk.Label(self.welcome_frame, text="Bienvenue sur le Projet Data Filter",
        ...
        change_bg_button = tk.Button(self.welcome_frame, text="Changer de fond", font=("Arial", 12),
    ...
    def create_main_app(self):
        ...
        tk.Button(button_frame, text="Annuler les Filtres", command=lambda: self.show_data()).grid(row=1, column=2,
                                                                                                   padx=10, pady=10)
    ...
    def show_stats(self):
        ...
        for key in self.data[0].keys():
            ...
            if numeric_values:
                stats_result[
                    key] = f"Min: {min(numeric_values)}, Max: {max(numeric_values)}, Moyenne: {sum(numeric_values) / len(numeric_values):.2f}"
            elif bool_values:
                true_count = sum(bool_values)
                false_count = len(bool_values) - true_count
                stats_result[
                    key] = f"True: {true_count / len(bool_values) * 100:.2f}%, False: {false_count / len(bool_values) * 100:.2f}%"
            elif list_sizes:
                stats_result[
                    key] = f"Min: {min(list_sizes)}, Max: {max(list_sizes)}, Moyenne: {sum(list_sizes) / len(list_sizes):.2f}"
    ...
    def filter_data(self):
        ...
        tk.Label(filter_window, text="Sélectionnez la clé, la valeur et l'opérateur pour filtrer :",
        ...
        operators = ["==", "!=", "<", ">", "<=", ">=", "startswith", "endswith", "contains", "above_mean",
                     "below_percentile"]
